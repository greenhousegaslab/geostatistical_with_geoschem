...
#     include "CMN_SIZE"   ! Size parameters
      ...
      ! Local Variables
      INTEGER              :: I, I0, IOS, J,  J0, L, N, M
      INTEGER              :: YYYY, MM, DD,  HH, SS
      REAL*4               :: TRACER(IIPAR,JJPAR,LLPAR)
      INTEGER              :: LAYER(LLPAR)
      CHARACTER(LEN=255)   :: FILENAME

      ! Local variables required for writing netcdf files
      INTEGER :: STAT, NCID, VAR1, XID, YID, ZID
      INTEGER :: LONID, LATID, VARID2

      ! For binary punch file, version 2.0
      REAL*4               :: LONRES, LATRES
      INTEGER              :: HALFPOLAR, NSKIP
...
!$OMP END PARALLEL DO
      ...
      WRITE( 6, 110 ) N_TRACERS,  MINVAL( TRACER ), MAXVAL( TRACER )
 110  FORMAT( 'Tracer ', i3, ': Min = ', es12.5, '  Max = ',  es12.5 )
         ...
         !===========================================================
         NSKIP = ( BYTES_PER_NUMBER * ( IIPAR * JJPAR * LLPAR ) ) + 
     &           END_OF_RECORD
...
!      CLOSE( IU_RST )
         ...
         ! Open file
         STAT = NF_CREATE( TRIM(FILENAME), NF_CLOBBER, NCID )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

         ! Define dimensions
         STAT = NF_PUT_ATT_TEXT( NCID, NF_GLOBAL, "MODELNAME", 
     &           10, MODELNAME)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_REAL( NCID, NF_GLOBAL, "LONRES", 
     &           NF_FLOAT, 1, LONRES)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_REAL( NCID, NF_GLOBAL, "LATRES", 
     &           NF_FLOAT, 1, LATRES)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL,"HALFPOLAR",
     &           NF_INT, 1, HALFPOLAR)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "CENTER180", 
     &           NF_INT, 1, CENTER180)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_TEXT( NCID, NF_GLOBAL, "CATEGORY", 
     &           8, CATEGORY)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "NTRACER", 
     &           NF_INT, 1, N)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_TEXT( NCID, NF_GLOBAL, "UNIT", 
     &           3, UNIT )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_REAL( NCID, NF_GLOBAL, "ZTAU0", 
     &           NF_FLOAT, 1, TAU)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_REAL( NCID, NF_GLOBAL, "ZTAU1", 
     &           NF_FLOAT, 1, TAU)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_TEXT( NCID, NF_GLOBAL, "RESERVED", 
     &           1, RESERVED)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "NI", 
     &           NF_INT, 1, IIPAR)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "NJ", 
     &           NF_INT, 1, JJPAR)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "NL", 
     &           NF_INT, 1, LLPAR)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "IFIRST", 
     &           NF_INT, 1, I0+1)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "JFIRST", 
     &           NF_INT, 1, J0+1)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_INT( NCID, NF_GLOBAL, "LFIRST", 
     &           NF_INT, 1,1)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_ATT_REAL( NCID, NF_GLOBAL, "NSKIP", 
     &           NF_INT, 1, NSKIP)
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

         ! Create a sequence of numbers to represent
         ! vertical levels in the model
         DO M = 1, LLPAR
            LAYER(M) = M
         ENDDO

         ! Set dimension variables
         STAT = NF_DEF_DIM( NCID, "lon", IIPAR, xID )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_DEF_DIM( NCID, "lat", JJPAR, yID )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_DEF_DIM( NCID, "layer", LLPAR, zID )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

         ! Define variables
         STAT = NF_DEF_VAR( NCID, "lon", NF_FLOAT,
     &           1, (/ xid /), lonID )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_DEF_VAR( NCID, "lat", NF_FLOAT,
     &           1, (/ yid /), latID )
         STAT = NF_DEF_VAR( NCID, "layer", NF_INT,
     &           1, (/ zid /), LAYER )
         STAT = NF_DEF_VAR( NCID, "tracer", NF_FLOAT,
     &           3, (/ xid, yid, zid /), VARID2 )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         ...
         ! Write the data
         STAT = NF_PUT_VAR_REAL( NCID, LATID, YMID )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_VAR_REAL( NCID, LONID, XMID )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
         STAT = NF_PUT_VAR_REAL( NCID, VARID2, TRACER )
         IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
...
!         IF ( IOS /= 0 ) CALL IOERROR( IOS,IU_RST,'read_restart_file:6')
      ...
      NCSTATUS = NF_OPEN( FILENAME, 0, NCID)
      NCSTATUS = NF_INQ_VARID( NCID, 'tracer', VARID )
      NCSTATUS = NF_GET_VAR_REAL( NCID, VARID, TRACER )
      NCSTATUS = NF_GET_ATT_TEXT( NCID, NF_GLOBAL, 'MODELNAME',
     & MODELNAME )
      NCSTATUS = NF_GET_ATT_REAL( NCID, NF_GLOBAL, 'LONRES', LONRES )
      NCSTATUS = NF_GET_ATT_REAL( NCID, NF_GLOBAL, 'LATRES', LATRES )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'HALFPOLAR',
     & HALFPOLAR )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'CENTER180',
     & CENTER180 )
      NCSTATUS = NF_GET_ATT_TEXT( NCID, NF_GLOBAL, 'CATEGORY',
     & CATEGORY )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'NTRACER', NTRACER )
      NCSTATUS = NF_GET_ATT_TEXT( NCID, NF_GLOBAL, 'UNIT', UNIT )
      NCSTATUS = NF_GET_ATT_REAL( NCID, NF_GLOBAL, 'ZTAU0', ZTAU0 )
      NCSTATUS = NF_GET_ATT_REAL( NCID, NF_GLOBAL, 'ZTAU1', ZTAU1 )
      NCSTATUS = NF_GET_ATT_TEXT( NCID, NF_GLOBAL, 'RESERVED',
     & RESERVED )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'NI', NI )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'NJ', NJ )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'NL', NL )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'IFIRST', IFIRST )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'JFIRST', JFIRST )
      NCSTATUS = NF_GET_ATT_INT(  NCID, NF_GLOBAL, 'LFIRST', LFIRST )
      NCSTATUS = NF_GET_ATT_REAL( NCID, NF_GLOBAL, 'NSKIP', NSKIP )
