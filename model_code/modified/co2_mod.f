...
!
      ...
      USE LOGICAL_ADJ_MOD, ONLY : LADJ, LADJ_EMS, LFDTEST !zc
...
#     include "netcdf.inc"                    ! Include file for netCDF
#     include "CMN_SIZE"      ! Size parameters
!
! !INPUT PARAMETERS:
!
...
!
      ...
      INTEGER                :: STD(3), CTD(3) !one dimensional integer array
      INTEGER                :: fId                ! netCDF file ID
      REAL*4                 :: ARRAY(IGLOB,JGLOB,1)
      CHARACTER(LEN=4  )     :: FYEAR
      CHARACTER(LEN=2  )     :: FMONTH
      CHARACTER(LEN=2  )     :: FDAY
      CHARACTER(LEN=255)     :: FILENAME
      ...
      WRITE( FYEAR, '(i4.4)' ) YEAR
      WRITE( FMONTH,'(i2.2)' ) MONTH
      WRITE( FDAY,  '(i2.2)' ) DAY

      ! Name of file with Balanced Bio CO2 data
      FILENAME = TRIM(FLUX_DIR) //
     &           TRIM('CO2.daily.') //
     &           GET_NAME_EXT_2D() // '.'              //
     &           GET_RES_EXT()     // '.'              //
     &           TRIM(FYEAR)       // '.'              //
 ...
 100  FORMAT( '     - READ_BBIO_DAILYAVERAGE: Reading ', a )
      ...
      ! Read CO2 fluxes
      V_NAME = "CO2_flux"
      CALL NcRd( EMBIOCO2, fId, TRIM(V_NAME), STD, CTD )

      ! Close netCDF file
      CALL NcCl( fId )

      print*, ' MAX FLUX VALUE = ', maxval(EMBIOCO2(:,:))
      print*, ' MIN FLUX VALUE = ', minval(EMBIOCO2(:,:))
