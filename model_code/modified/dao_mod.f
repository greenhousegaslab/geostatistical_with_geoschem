...
!  22 Oct 2015 - M. Sulprizio- Add support for MERRA2 meteorology
!******************************************************************************
!
      IMPLICIT NONE
...
!------------------------------------------------------------------------------
      ...
      FUNCTION GET_WET_SURFACE_PRESSURE(ILON, ILAT) RESULT(wetPS)
...
! local variables
      real*8    :: p1, p2, wetPS, sp
      real*8    :: delp2, delP2Mass, delpDry, delpWet
      integer   :: L
      ! real*8    :: PTOP       = 0.01d0
      delP2Mass = G0_100 * GET_AREA_M2(ILAT)

      wetPS = PTOP
      DO L = 1, LLPAR
        delpDry = AD(ILON, ILAT, L) / delP2Mass
        sp = (1-SPHU(ILON, ILAT, L) * 1.0e-3)
        delpWet = delpDry/sp
        wetPS = wetPS + delpWet
        !print *, delpDry, delpWet
      ENDDO

      END FUNCTION GET_WET_SURFACE_PRESSURE
!------------------------------------------------------------------------------

      SUBROUTINE GET_WET_PRESSURE_PROFILE(ILON, ILAT, PROFILE)
!
! Compose a full pressure profile based on the dry surface pressure
! by adding the water vapor pressure 
!
...
! arguments
      integer, intent(in)  :: ILAT, ILON
      real*8, intent(out)  :: PROFILE(LLPAR)

! local variables
      integer :: L
      real*8  :: wetPS

      wetPS = GET_WET_SURFACE_PRESSURE(ILON, ILAT)

      ! Scot: I find the mid-points of the pressure levels in this
      ! subroutine.
      DO L = 1, LLPAR
         profile(L) = 0.5d0 * ((GET_AP(L) + GET_BP(L)*wetPS)  
     &       + (GET_AP(L+1) + GET_BP(L+1)*wetPS))
      ENDDO

      END SUBROUTINE GET_WET_PRESSURE_PROFILE
...
!! (lzh, 11/01/2014)
#if   defined( GRID1x1 ) || defined( GRID05x0666 ) || defined( GRID05x0625) || defined( GRID025x03125 )
...
!!! (lzh, 04/09/2014)
      ...
      !=================================================================
      ! Do the interpolation for GEOS-FP and MERRA2 met fields 
      !=================================================================
!$OMP PARALLEL DO
!$OMP+DEFAULT( SHARED )
!$OMP+PRIVATE( I, J, L )
...
#elif defined( GEOS_4 ) || defined( GEOS_5 ) || defined(GEOS_FP ) || defined( MERRA2 )
...
#elif defined( GEOS_5 ) || defined( GEOS_FP ) || defined( MERRA2 )
