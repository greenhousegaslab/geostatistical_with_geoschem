...
#     include "define_adj.h"     ! the obs operators
      ...
      IF ( LFDTEST ) THEN
         ...
         IF ( LCSPEC_OBS ) OBS_THIS_SPECIES(:)    = .FALSE.

         OBS_THIS_TRACER(NFD)   = .TRUE.

         IF ( LCSPEC_OBS ) OBS_THIS_SPECIES(NFD) = .TRUE.
...
!********************************************************************************
      ...
      ! Reference to f90 modules
      USE TIME_MOD,       ONLY : GET_DAY, GET_DIRECTION
...
#     include "CMN_SIZE" ! Size stuff
      ...
      INTEGER, SAVE :: DAY_SAVE
      INTEGER, SAVE :: GROUP_SAVE
      LOGICAL, SAVE :: FIRST_FWD = .TRUE.
      LOGICAL, SAVE :: FIRST_BWD = .TRUE.
      ...
      IF (GET_DIRECTION() .eq. 1) THEN
         IF (FIRST_FWD) THEN 
           ...
           FIRST_FWD = .FALSE.
           print *, 'FWD: group initialized as ', CURRENT_GROUP
         ENDIF
         IF ( DAY_SAVE /= GET_DAY() ) THEN 
           DAY_SAVE = GET_DAY()
           GROUP_SAVE = GROUP_SAVE + 1
           CURRENT_GROUP = GROUP_SAVE
           print *, 'group modified as ', CURRENT_GROUP
         ELSE
           CURRENT_GROUP = GROUP_SAVE
         ENDIF
      ELSE
         IF (FIRST_BWD) THEN 
           ...
           CURRENT_GROUP = GROUP_SAVE
           FIRST_BWD = .FALSE.
           print *, 'BWD: group initialized as ', CURRENT_GROUP
         ENDIF
         IF ( DAY_SAVE /= GET_DAY() ) THEN 
           DAY_SAVE = GET_DAY()
           GROUP_SAVE = GROUP_SAVE - 1
...
#     include "comode.h"   ! NSPEC, NAMEGAS
      ...
      DO N = 1, NSPEC(NCSURBAN)
         ...
         IF ( TRIM(NAMEGAS(N)) == TRIM( SPEC_NAME ) ) I = N
      ENDDO
