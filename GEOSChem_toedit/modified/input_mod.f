...
!  23 Oct 2015 - M. Sulprizio  - Add support for MERRA2 meteorology
!******************************************************************************
!
      IMPLICIT NONE
      ...
      CHARACTER(LEN=255) :: FILENAME2 = 'input.noedit'
...
!
 ...
 100  FORMAT( 'READ_INPUT_FILE: Reading ', a )
      ...
      !=========================================================
      ! Read additional input files (in this case, input.noedit)
      ! Open file
      OPEN( IU_GEOS, FILE=TRIM( FILENAME2 ), STATUS='OLD', IOSTAT=IOS )
      IF ( IOS /= 0 ) CALL IOERROR( IOS, IU_GEOS, 'read_input_file:1' )
      ...
      DO
         ...
         IF ( INDEX( LINE, 'TRACER MENU'           ) > 0 ) THEN
...
!
      ...
      USE DIRECTORY_MOD, ONLY : MERRA2_DIR    !! (mps,10/23/2015) merra2
      USE DIRECTORY_MOD, ONLY : RUN_DIR
      USE DIRECTORY_MOD, ONLY : TEMP_DIR
      USE DIRECTORY_MOD, ONLY : OBS_DIR_OCO2, OBS_DIR_FLASK
      USE DIRECTORY_MOD, ONLY : FFILE_PREFIX, OCOFILE_PREFIX
      USE DIRECTORY_MOD, ONLY : FLUX_DIR
      USE ADJ_ARRAYS_MOD,ONLY : FLASKQFLAG, MMSCL
      ...
      ! OCO2 observation directory
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_simulation_menu:13'  )
      READ( SUBSTRS(1:N), '(a)' ) OBS_DIR_OCO2

      ! Prefix of OCO2 observation files
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_simulation_menu:14'  )
      READ( SUBSTRS(1:N), '(a)' ) OCOFILE_PREFIX

      ! Flask observation directory
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_simulation_menu:15'  )
      READ( SUBSTRS(1:N), '(a)' ) OBS_DIR_FLASK

      ! Prefix of flask observation files
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_simulation_menu:16'  )
      READ( SUBSTRS(1:N), '(a)' ) FFILE_PREFIX

      ! Quality flag for flask obs (1 = only QF=1; 0 = include all flask
      ! data)
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_simulation_menu:17'  )
      READ( SUBSTRS(1:N), * ) FLASKQFLAG
      ...
      READ( SUBSTRS(1:N), '(a)' ) FLUX_DIR

      ! Read the number of time periods in the inverse model (MMSCL)
     ...
     &                     TRIM( GEOS_FP_DIR )
      WRITE( 6, 110     ) 'MERRA2     sub-directory    : ',
     &                     TRIM( MERRA2_DIR )
...
!
      ...
      USE DIRECTORY_MOD, ONLY : GEOS_FP_DIR, MERRA2_DIR
...
!
      ...
      USE DIRECTORY_MOD, ONLY : GEOS_5_DIR, MERRA2_DIR, GEOS_FP_DIR
