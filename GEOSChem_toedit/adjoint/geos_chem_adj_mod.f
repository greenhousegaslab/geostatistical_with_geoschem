...
!      SUBROUTINE DO_GEOS_CHEM_ADJ
      ...
      N_DYN_STEPS = 180 / GET_TS_DYN()     ! GEOS-FP/MERRA2 have a 3-hr interval
#else
...
! merra2 (mps,10/26/2015)
         ...
         IF ( ITS_TIME_FOR_I3_ADJ() ) THEN
            ...
            !            ***** C O P Y   I - 3   F I E L D S *****
            !
            !        The I-6 fields at the beginning of the next (
            !        forward )
            !        timestep become the fields at the end of this
            !        timestep
            !=================================================================
            CALL COPY_I6_FIELDS_ADJ
            ! Get the date/time for the previous I-6 data block
            BEHIND_DATE = GET_I3_TIME_ADJ()
            ! Open and read files
            CALL MERRA2_READ_I3_1( BEHIND_DATE(1), BEHIND_DATE(2) )
!            CALL OPEN_I6_FIELDS_ADJ(  BEHIND_DATE(1), BEHIND_DATE(2) )
!            CALL GET_I6_FIELDS_1_ADJ( BEHIND_DATE(1), BEHIND_DATE(2) )
            PRINT*,'I3 DATE = ',BEHIND_DATE(1),BEHIND_DATE(2)
            ! Compute avg pressure at polar caps (for ADJ argument is
            ! PS1, not PS2)
            CALL AVGPOLE( PS1 )
            ! Set NSECb_ADJ to be used for the interpolation
            ! where NSECb_ADJ is the total elapsed time in seconds at
            ! the
            ! beginning of the current 6h time step which contains
            ! ELAPSED_MIN
            NSECb_ADJ = ( MIN_ADJ + GET_TS_DYN() ) * 60  - 3 * 3600
         ENDIF
...
! merra2 (mps, 10/26/2015)
         ...
         IF ( ITS_TIME_FOR_A3_ADJ() ) THEN
            ...
            BEHIND_DATE = GET_A3_TIME_ADJ()
            ! Open and read files
            CALL MERRA2_READ_A3( BEHIND_DATE(1), BEHIND_DATE(2) )
         ENDIF
...
! merra2 (mps, 10/26/2015)
         ...
         IF ( ITS_TIME_FOR_A1_ADJ() ) THEN
            ...
            BEHIND_DATE = GET_A1_TIME_ADJ()
            ! Open & read A-3 fields
            CALL MERRA2_READ_A1( BEHIND_DATE(1), BEHIND_DATE(2) )
!            CALL OPEN_A3_FIELDS_ADJ( BEHIND_DATE(1), BEHIND_DATE(2) )
!            CALL GET_A3_FIELDS(  BEHIND_DATE(1), BEHIND_DATE(2) )
            ! Update daily mean temperature archive for MEGAN biogenics
            ! For adjoint, read in checkpointed values (dkh, 01/23/10)
...
!         IF ( LADJ_TRAJ ) THEN
         ...
         IF ( LADJ_TRAJ .and. ( ITS_TIME_FOR_A1() ) ) THEN
            ...
            !CALL MAKE_GDT_MULTI_FILE
            IF (GET_NHMS() == 0) THEN 
            ! Note from Scot: the functions below don't exist in
            ! checkpt_mod.f, so I've commented them out below.
            ! CALL MAKE_EMS_ADJ_FILE_ZZ1( GET_NYMD(), GET_NHMS() )!ZZ
            ! CALL MAKE_EMS_ADJ_FILE_ZZ2( GET_NYMD(), GET_NHMS() )!ZZ
            ! CALL MAKE_EMS_ADJ_FILE_ZZ3( GET_NYMD(), GET_NHMS() )!ZZ
            ENDIF
         ENDIF
...
!     &   MINVAL(ICS_SF_ADJ), MAXVAL(ICS_SF_ADJ)
      ...
      ! Scot: the prior is only added to the cost function and gradient
      ! if the "LAPSRC" option is set in input.gcadj. Otherwise, the
      ! prior term is omitted (which is what we want)
      IF ( LAPSRC ) THEN
...
#if defined(INSITU_CO2_OBS)
      USE FLASK_CO2_MOD,       ONLY : CALC_FLASK_CO2_FORCE
#endif
...
!      ENDIF
      ...
      !================================================================
      ! CO2 IN SITU OBSERVATIONS FROM THE OBSPACK
      !================================================================

#if   defined ( INSITU_CO2_OBS )

      CALL CALC_FLASK_CO2_FORCE( COST_FUNC )
...
!
      ...
      USE TIME_MOD,             ONLY : GET_TS_DYN, GET_NYMD
      USE TIME_MOD,             ONLY : GET_TS_CHEM, GET_NHMS
...
#     include "comode.h"             ! IGAS, ITLOOP
      ...
      ! Stuff for checking clouds                                                                                                               
      integer :: res, ix, cnt                                                                                                                   
!      real, dimension(300000,52) :: data_array                                                                                                   
      integer :: line_no, iostat, stat

      ! Variables for interpolation
      INTEGER, PARAMETER           :: MAXLEV = 71                                                               
      INTEGER, PARAMETER           :: L_OBS = 71                                                                
                                                                                                                
      integer :: vals                                                                                          \
                                                                                                                
      real, dimension(2957,5) :: data_array                                                                    
      REAL*8                   :: GC_PRES(LLPAR)  
      REAL*8                   :: GC_CO2_NATIVE(LLPAR)
      REAL*8                   :: OBS_PRES(L_OBS)                                                               
      REAL*8                   :: OBS_CO2_FCD(L_OBS)                                                            
      REAL*8                   :: OBS_CO2(L_OBS)                                                                
      REAL*8                   :: AK(L_OBS)                                                                     
      REAL*8                   :: CO2(LLPAR)                                                                    
      REAL*8                   :: CHK_STT_AK(LLPAR)                                                                    
      REAL*8              :: GC_PSURF                                                                           
      REAL*8              :: MAP_BACK(MAXLEV,LLPAR)                                                             
...
!$OMP+PRIVATE( DIFF )
         DO NN = 1, NOBS
            IF( (GET_NYMD()== 20100201) .and. !BB bbyrne
     &           (GET_NHMS() == 000000) )  THEN

               print*, 'time: ',GET_NYMD(),' --- ',GET_NHMS()
...
!      ENDDO                                                                                  
     ...
     &           TM_PRESC(LTM)  > LOW) THEN                                                   
       ...
       ! DO THE SAME THING FOR THE HIGHEST LEVEL                                                                
       ! New code:                                                                                              
       ! Loop over each pressure level of TM grid                                                               
       DO LTM = 1, LTM_TOP                                                                                      
          IF ( TM_PRESC(LTM) < GC_PRESC(LGC_TOP) ) THEN                                                         
             HINTERPZ(LGC_TOP,LTM)     = 1D0                                                                    
             HINTERPZ(1:LGC_TOP-1,LTM) = 0D0                                                                    
          ENDIF                                                                                                 
       ENDDO                                                          
...
#     include "define.h"     ! ITLOOP, IGAS
      ...
      IF ( FIRST ) THEN
         SLP_TMP(:,:) = SLP(:,:)
#if   defined( GEOS_3 ) || defined( GEOS_4 ) || defined( GEOS_5 ) || defined(GEOS_FP) || defined( MERRA2 )
         LWI_TMP(:,:) = LWI(:,:)
#endif
#if   defined( GEOS_5 ) || defined(GEOS_FP) || defined( MERRA2 )
         TO3_TMP(:,:)  = TO3(:,:)
...
#     include "CMN_SIZE"
      ...
      ! Only runs if following option is set in input.gcadj 
...
!$OMP+PRIVATE( I, J, M, N, REG, S2_INV )
     ...
     &                         * REG ** 2
               ...
               EMS_SF_ADJ(I,J,M,N) = EMS_SF_ADJ(I,J,M,N)!zc prior delete
