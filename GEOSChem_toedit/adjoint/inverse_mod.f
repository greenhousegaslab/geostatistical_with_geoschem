...
#     include "define_adj.h"    ! obs operators
      ...
      LOGICAL, PARAMETER   :: NETCDF_IO = .TRUE.
...
#     include "netcdf.inc"
      ...
      INTEGER :: STAT, NCID, XID, YID, ZID, TID, SID, T2ID
      INTEGER :: LATID, TIMEID, MONID, YEARID, LONID, LEVID
      INTEGER :: VARID, VARID2, VARID3
      INTEGER :: MONTHS(MMSCL)
     ...
     &                  J0+1,      1,         RATE_3D )
      ...
      ELSE
         ...
         IF ( LADJ_EMS ) THEN
            ...
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            STAT = NF_DEF_DIM( NCID, "time", MMSCL, tID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            STAT = NF_DEF_DIM( NCID, "species", NNEMS, sID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

            ! Define variables
            STAT = NF_DEF_VAR( NCID, "lon", NF_DOUBLE,
     &           1, (/ xid /), lonID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            STAT = NF_DEF_VAR( NCID, "lat", NF_DOUBLE,
     &           1, (/ yid /), latID )
            STAT = NF_DEF_VAR( NCID, "time", NF_INT,
     &           1, (/ tid /), monID )
            STAT = NF_DEF_VAR( NCID, "dJ_dESF", NF_DOUBLE,
     &           4, (/ xid, yid, tid, sid /), VARID2 )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            ...
            ! Write the data
            STAT = NF_PUT_VAR_DOUBLE( NCID, LATID, YMID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            STAT = NF_PUT_VAR_DOUBLE( NCID, LONID, XMID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            ...
            STAT = NF_PUT_VAR_INT( NCID, monID, MONTHS )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

            STAT = NF_PUT_VAR_DOUBLE( NCID, VARID2, EMS_SF_ADJ )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
     ...
     &     ' Invalid number RRATES found ' , 'READ_GDT_FILE' )
      ...
      ELSE
         ...
         IF ( LADJ_EMS ) THEN
            ...
            WRITE(6,*) 'Reading in gdt...'
            STAT = NF_OPEN( FILENAME, NF_NOWRITE,NCID)
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

            STAT = NF_INQ_VARID( NCID, 'dJ_dESF', VARID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

            STAT = NF_GET_VAR_DOUBLE( NCID, VARID, EMS_SF_ADJ )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
...
#     include "netcdf.inc"
      ...
      INTEGER :: STAT, NCID, XID, YID, TID, SID
      INTEGER :: LATID, LONID, MONID, VARID, VARID2
      INTEGER :: MONTHS(MMSCL)
...
!$OMP END PARALLEL DO
     ...
     &           1, (/ xid /), lonID )
            ...
            STAT = NF_DEF_VAR( NCID, "lat", NF_DOUBLE,
     &           1, (/ yid /), latID )
            STAT = NF_DEF_VAR( NCID, "month", NF_INT,
     &           1, (/ tid /), monID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            STAT = NF_DEF_VAR( NCID, "E", NF_DOUBLE,
     &           4, (/ xid, yid, tid, sid /), VARID2 )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            ...
            STAT = NF_PUT_VAR_DOUBLE( NCID, VARID2, EMS_SF )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
...
!$OMP END PARALLEL DO
      ...
      ELSE
         ...
         IF ( LADJ_EMS ) THEN
            ...
            WRITE(6,*) 'Reading in sf...'
            STAT = NF_OPEN( FILENAME, NF_NOWRITE,NCID)
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"

            STAT = NF_INQ_VARID( NCID, 'E', VARID )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
            STAT = NF_GET_VAR_DOUBLE( NCID, VARID, EMS_SF )
            IF ( STAT .ne. NF_NOERR ) WRITE(6,*) "Error"
...
#     include "define_adj.h"        ! Obs operator flags
      ...
      ! References to F90 modules
#if defined(OMI_NO2_OBS) || defined(TES_O3_OBS) || defined(MOPITT_V5_CO_OBS) || defined (MOPITT_V6_CO_OBS )
      USE HDF5
#endif
