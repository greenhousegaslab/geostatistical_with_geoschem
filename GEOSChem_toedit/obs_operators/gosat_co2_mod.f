...
!
      ...
      USE DIRECTORY_MOD,          ONLY : OCOFILE_PREFIX
      USE NETCDF 
      USE TIME_MOD
	...
	print *, 'OBS_DIR_OCO2=', OBS_DIR_OCO2
	write(READ_filename,102)TRIM(OBS_DIR_OCO2),TRIM(OCOFILE_PREFIX),
...
102    format(a,a,i4.4,i2.2,i2.2,a)
     ...
     &,xCO2_ID ), 116)
      ! READ quality flag 
      CALL CHECK( NF90_INQ_VARID(FID,"assimilate_flag", QF_ID ),114)
...
#     include      "CMN_SIZE"      ! Size params
      ...
      character(LEN=255)          :: filenameobs='residuals_oco2.csv'
      REAL*8                      :: outvec(7)
 ...
 100  FORMAT(1X,F16.8,1X,F20.8)
	...
	!write(6,*)'xPrior=', GOS(NT)%xPRIOR, 'CO2_HAT=', CO2_HAT(1)
	  ...
	  !write(filenameobs(8:15),'(I8)')YYYYMMDD
          ...
          outvec = (/GOS(NT)%SID,GOS(NT)%lat(1),GOS(NT)%lon(1),
     &                    CO2_HAT(1:nlev)*1e6,GOS(NT)%xCO2*1e6,
     &                    GOS(NT)%xstd*1e6,GOS(NT)%QF(1)/)
