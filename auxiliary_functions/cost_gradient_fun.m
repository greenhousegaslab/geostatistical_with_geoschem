...
function [ f,g ] = cost_gradient_fun( X, A, B, Qinv, Qsqrt, shat, landall, year, geosdir, fluxpath, ocean, ntimes);
...
%----------------------------------------------------------------------------------%
	...
	write_fluxes(fluxpath, landall, ocean, ntimes, shat_backtransform, year);
...
%---------------------------------------------------------------------------%
	...
	% Check to see if the gradient file (gctm.gdt.01) exists. If not, print a warning message.	
	if ~exist( 'OptData/gctm.gdt.01.nc4', 'file' )
		disp('ERROR: GRADIENT FILE DOES NOT EXIST.');
		disp('GEO-SCHEM MOST LIKELY CRASHED.');
	end;

	adj = ncread('OptData/gctm.gdt.01.nc4','dJ_dESF');

	% The gradient file for CO2 contains multiple tracers -- I think they're supposed to 
	% correspond to different source types. We'll sum the gradient across these tracers.
	adj = sum(adj,4);
...
%------------------------------------------------------%
	...
	% Print an error message of the observation component of the gradient is zero.
	if mean(L1) == 0;
		disp('ERROR: THE OBSERVATION COMPONENT OF THE GRADIENT IS ZERO.');
		disp('THE NUMBER OF TIME PERIODS SPECIFIED IN INPUT.GEOS MAY BE WRONG.');
		disp('OR THE MODEL ISNT READING IN THE GHG OBSERVATIONS CORRECTLY.');
