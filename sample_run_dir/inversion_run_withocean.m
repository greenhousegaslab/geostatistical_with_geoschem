...
%-------------------------------%
	...
	fluxpath = '/scratch/groups/smill191/smiller/outputs/OCO2_MIP/fluxes_LNLG/';
...
%----------------------------%
	...
	load(strcat('~/work/smiller/data/OCO2_MIP/CASA_GFED4/regridded/','CASA_',num2str(yr1),'.mat'));

	% Read in ODIAC fossil fuel emissions
	load(strcat('~/work/smiller/data/OCO2_MIP/ODIAC/regridded/','ODIAC_',num2str(yr),'.mat'));

	% Read in GFED biomass burning emissions
	load(strcat('~/work/smiller/data/OCO2_MIP/GFED/regridded/','gfed_',num2str(yr),'.mat'));

	% Read in ocean fluxes 	
	load(strcat('~/work/smiller/data/OCO2_MIP/ECCO_DARWIN/','ocean_',num2str(yr2),'.mat'));
	...
	NEE2014 = load('~/work/smiller/data/OCO2_MIP/CASA_GFED4/regridded/CASA_2014.mat');
	NEE2014 = NEE2014.NEE;
	...
	odiac2014 = load(strcat('~/work/smiller/data/OCO2_MIP/ODIAC/regridded/','ODIAC_2014.mat'));
	odiac = [odiac2014.odiac; odiac]; 

	% GFED: Existing flux files include a full year of fluxes for 2014
	gfed2014 = load(strcat('~/work/smiller/data/OCO2_MIP/GFED/regridded/','gfed_2014.mat'));
...
%-------------------------%
	...
	% Note: The line above could create values of "0" in the sigmaQ vector below. That would yield a matrix Q that is not positive definite.
	% (The inverse model requires covariance matrices that are positive definite.)  
...
%----------------------------%
	...
	% datasets, e.g., Miller 2018 & 2020;
    	...
    	D1 = exp(-1 .* days ./ theta1(3));  Dinv1 =inv(D1);
...
%-------------------------%
	...
	f1 = @(shat) cost_gradient_fun(X, A, B, Einv, Dinv, Dinv1, CD, CD1, CE, shat, landmap, yr, geosdir, fluxpath); 
...
%------------------------------%
        ...
        outname = strcat(fluxpath,'fluxes_LBFGS',num2str(maxit),'.csv');
