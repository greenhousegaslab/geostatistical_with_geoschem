...
%	    OPTIONS.HessUpdate : If set to 'bfgs', BroydenñFletcherñGoldfarbñShanno 
...
defaultopt = struct('Display','final','HessUpdate','lbfgs','GoalsExactAchieve',1,'GradConstr',false,  ...
			'TolX',1e-6,'TolFun',1e-6,'GradObj','on','MaxIter',400,'MaxFunEvals',100*numel(x_init)-1,  ...
			'DiffMaxChange',1,'DiffMinChange',1e-8,'OutputFcn',[], ...
			'rho',0.0100,'sigma',0.900,'tau1',3,'tau2', 0.1, 'tau3', 0.5,'StoreN',20,'restart',false);
...
% Initialize data structure if the current run is not a restart run
if(~optim.restart)

disp('This run is not a restart run. Start from iteration 1.');
...
else; % if/else statement for restart run

disp('This run is a restart run. Start where the previous run left off.');

% If a partial run already exists, read in the most recent data object from that run
load('./data.mat');
...
%    save(strcat(outpath,'shat_',num2str(data.iteration),'.mat'),'shat');
    ...
    % Save the data object from this iteration in case the run needs to be restarted
    save('./data.mat','data');
...
if(optim.HessUpdate(1)=='b'), output.algorithm='BroydenñFletcherñGoldfarbñShanno (BFGS)';
